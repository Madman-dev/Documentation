 ...
 */
var registrationList: [String] = [] // initialization for Array is like creating an instance
...
registrationList += ["Jack", "Steve", "Dad", "Mom"] // 많은 데이터를 담을 때는 += 를 활용해서 적으면 된다~
print(registrationList)
//:  Use the `insert(_:at:)` method to add `Charlie` into the array as the second element. Print the contents of the collection.
registrationList.insert("Charlie", at: 1)   // insert를 할 때는 []를 사용하는게 아니다~
print(registrationList)
...
print(registrationList)     // 마지막, 끝에 있는 데이터 값은 -1을 해야한다.
